...
# -----------------------------
rgb(10, 4, 23, maxColorValue = 255, alpha = 10) # alpha는 투명도
col2rgb('lightblue')
...
contour(x, y, volcano, levels = seq(90, 200, by = 5),
        add = TRUE, col = 'white') # 등고선을 올림, white가 큰 값

x <- 10*(1:nrow(volcano))
y <- 10*(1:ncol(volcano))
image(x, y, volcano, col = heat.colors(20, alpha = 1), axes = FALSE)
contour(x, y, volcano, levels = seq(90, 200, by = 5), reverse=?,
        add = TRUE, col = 'white') # 등고선을 올림, white가 작은 값으로

x <- 10*(1:nrow(volcano))
y <- 10*(1:ncol(volcano))
image(x, y, volcano, col = topo.colors(20, alpha = 1), axes = FALSE)
contour(x, y, volcano, levels = seq(90, 200, by = 5),
        add = TRUE, col = 'white') # 등고선을 올림
